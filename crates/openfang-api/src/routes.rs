...
fn infer_attachment_extension(filename: &str, content_type: &str) -> Option<String> {
    let mime_ext = match content_type {
        "image/jpeg" => Some("jpg"),
        "image/png" => Some("png"),
        "image/gif" => Some("gif"),
...
fn ensure_attachment_tool_path(
    file_id: &str,
    filename: &str,
    content_type: &str,
) -> Option<String> {
    ...
    let raw_path = upload_dir().join(file_id);
    if !raw_path.exists() {
        return None;
    }

    let Some(ext) = infer_attachment_extension(filename, content_type) else {
        return Some(raw_path.to_string_lossy().to_string());
    };

    let derived_path = upload_dir().join(format!("{file_id}.{ext}"));
    if !derived_path.exists() {
        if let Err(e) = std::fs::copy(&raw_path, &derived_path) {
            tracing::warn!(
                file_id = %file_id,
                path = %derived_path.display(),
                error = %e,
                "Failed to create tool-friendly attachment alias"
            );
            return Some(raw_path.to_string_lossy().to_string());
        }
    }

    Some(derived_path.to_string_lossy().to_string())
}

fn resolve_attachment(att: &AttachmentRef) -> Option<ResolvedAttachment> {
    let meta = UPLOAD_REGISTRY.get(&att.file_id);
    let filename = meta
        .as_ref()
        .map(|m| m.filename.clone())
        .filter(|s| !s.trim().is_empty())
        .or_else(|| (!att.filename.trim().is_empty()).then(|| att.filename.clone()))
        .unwrap_or_else(|| "upload".to_string());
    let content_type = meta
        .as_ref()
        .map(|m| m.content_type.clone())
        .filter(|s| !s.trim().is_empty())
        .or_else(|| (!att.content_type.trim().is_empty()).then(|| att.content_type.clone()))
        .unwrap_or_else(|| "application/octet-stream".to_string());
    let local_path = ensure_attachment_tool_path(&att.file_id, &filename, &content_type)?;
...
fn attachment_kind(content_type: &str) -> &'static str {
...
fn build_attachment_context_lines(
    resolved: &[ResolvedAttachment],
    total_count: usize,
    has_inline_media: bool,
) -> Vec<String> {
    let mut lines = vec!["[附件上下文]".to_string()];
    if has_inline_media {
        lines.push(
            "本轮消息中的图片/音频/视频已直接包含，可直接理解和使用。以下为未能内联的附件引用："
                .to_string(),
        );
    } else {
        lines.push(
    ...
    lines.push(
        "如果没有真实调用发送工具成功，不得声称「已经发给你」或「刚发到飞书了」。".to_string(),
    );
    for (idx, att) in resolved.iter().enumerate() {
        lines.push(format!(
            "- attachment_{idx}: kind={}; filename={}; content_type={}; file_id={}; local_path={}",
            attachment_kind(&att.content_type),
            att.filename,
            att.content_type,
            att.file_id,
            att.local_path,
...
/// Read image attachments and encode as base64 ContentBlock::Image for
/// direct LLM perception. Non-image attachments (audio, video, etc.) are
/// transcribed / described via the media engine and returned as text blocks.
/// Attachments that cannot be inlined are left for reference-only context.
pub(crate) async fn resolve_media_blocks(
    ...
    let resolved: Vec<ResolvedAttachment> = attachments
        .iter()
        .take(16)
        .filter_map(resolve_attachment)
        .collect();

    let mut media_blocks: Vec<ContentBlock> = vec![];
    let mut ref_only: Vec<ResolvedAttachment> = vec![];

    for att in &resolved {
        let kind = attachment_kind(&att.content_type);
        match kind {
            "image" => {
                // Read and base64-encode for direct LLM perception
                match tokio::fs::read(&att.local_path).await {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            ...
                            path = %att.local_path,
                            error = %e,
                            "Failed to read image for inline encoding, falling back to reference"
                        );
                        ref_only.push(att.clone());
                    }
                }
            }
            "audio" | "video" => {
                // Transcribe/describe via media engine; include result as text
                if let Some(engine) = media_engine {
                    let media_type = if kind == "audio" {
                        MediaType::Audio
                    } else {
                        MediaType::Video
                    };
                    match tokio::fs::metadata(&att.local_path).await {
                        Ok(meta) => {
                            let ma = MediaAttachment {
                                media_type,
                                mime_type: att.content_type.clone(),
                                source: MediaSource::FilePath {
                                    path: att.local_path.clone(),
                                },
                                size_bytes: meta.len(),
                            };
                            let result = if kind == "audio" {
                                engine.transcribe_audio(&ma).await
                            } else {
                                engine.describe_video(&ma).await
                            };
                            match result {
                                Ok(understanding) => {
                                    ...
                                    media_blocks.push(ContentBlock::Text {
                                        text: format!(
                                            ...
                                            label,
                                            att.filename,
                                            understanding.description.trim()
                                        ),
                                    });
                                }
                                Err(e) => {
                                    tracing::warn!(
                                        file_id = %att.file_id,
                                        error = %e,
                                        "Media understanding failed, keeping as reference"
                                    );
                                    ref_only.push(att.clone());
                                }
                            }
    ...
    (media_blocks, ref_only)
}

/// Build the text portion of the user message with attachment reference context.
///
/// `ref_only` are attachments that were not inlined as media blocks.
/// `has_inline_media` indicates whether some attachments were already inlined.
pub(crate) fn build_attachment_text_context(
    message: &str,
    ref_only: &[ResolvedAttachment],
    total_count: usize,
    has_inline_media: bool,
) -> String {
    if ref_only.is_empty() && !has_inline_media {
        return message.to_string();
    }

    if ref_only.is_empty() {
        // All attachments were inlined — no reference section needed
        return message.to_string();
    }

    format!(
        "{message}\n\n{}",
        build_attachment_context_lines(ref_only, total_count, has_inline_media).join("\n")
    )
}
...
mod attachment_context_tests {
    ...
    #[test]
    fn build_attachment_text_context_lists_reference_only_uploads() {
        let file_id = uuid::Uuid::new_v4().to_string();
        let raw_path = upload_dir().join(&file_id);
        let derived_path = upload_dir().join(format!("{file_id}.jpg"));
        let _ = std::fs::create_dir_all(upload_dir());
        std::fs::write(&raw_path, b"fake-image").expect("write raw upload");
        ...
        let ref_only = vec![ResolvedAttachment {
            file_id: file_id.clone(),
            filename: "photo.jpeg".to_string(),
            content_type: "image/jpeg".to_string(),
            local_path: derived_path.to_string_lossy().to_string(),
        }];

        let rendered = build_attachment_text_context("hello", &ref_only, 1, false);

        assert!(rendered.starts_with("hello\n\n[附件上下文]"));
        assert!(rendered.contains("需要时可调用工具读取"));
        assert!(rendered.contains("kind=image"));
        assert!(rendered.contains(&format!("file_id={file_id}")));
...
pub async fn send_message(
    ...
    let (media_blocks, ref_only) =
        resolve_media_blocks(&req.attachments, Some(&state.kernel.media_engine)).await;
    let message = build_attachment_text_context(
        &req.message,
        &ref_only,
        req.attachments.len(),
    ...
    }).await
    {
        Ok(result) => {
            ...
            // But respect silent flag — agent intentionally chose NO_REPLY (e.g. after sending voice via MCP)
            let response = if result.silent {
                String::new()
            } else if result.response.trim().is_empty() {
                format!(
...
struct UploadResponse {
    ...
    content_type: String,
    size: usize,
    /// Reserved for compatibility; media is no longer auto-transcribed on upload.
...
pub async fn upload_file(
    ...
    let _ = ensure_attachment_tool_path(&file_id, &filename, &content_type);
