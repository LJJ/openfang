...
pub async fn execute_tool(
            ...
            if mcp::is_mcp_tool(other) {
                if let Some(mcp_conns) = mcp_connections {
                    if let Some(server_name) = mcp::extract_mcp_server(other) {
                        ...
                        if other.ends_with("send_audio_message") {
                            if let Some(agent_id) = caller_agent_id {
                                if let Some(obj) = mcp_input.as_object_mut() {
                                    obj.entry("caller_agent_id".to_string()).or_insert_with(|| {
                        ...
                        if let Some(conn) = conns.iter_mut().find(|c| c.name() == server_name) {
                            ...
                            match conn.call_tool(other, &mcp_input).await {
    ...
    match result {
        Ok(content) => ToolResult {
            ...
            content,
            is_error: false,
            response_delivered: is_response_delivering_tool(tool_name),
        },
        Err(err) => ToolResult {
            tool_use_id: tool_use_id.to_string(),
...
/// Tools that deliver the response directly to the user via a side-channel
/// (e.g. voice/video message sent through MCP), bypassing the normal text response path.
pub(crate) fn is_response_delivering_tool(tool_name: &str) -> bool {
    // MCP tools that send messages directly to the user
    let delivering_suffixes = ["send_audio_message", "send_video_message"];
    delivering_suffixes
        .iter()
